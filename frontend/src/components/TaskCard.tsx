...
export default function TaskCard({ task, onEdit, onDelete, onStatusChange }: Props) {
  ...
  return (
    <motion.div
      ...
      className="card group hover:border-accent/40 transition-colors duration-200"
    >
      {/* Header row */}
      <div className="flex items-start justify-between gap-3 mb-3">
        <h3 className={`font-display font-semibold text-white leading-snug ${task.status === 'done' ? 'line-through opacity-50' : ''}`}>
          {task.title}
        </h3>
        <div className="flex items-center gap-1.5 shrink-0 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
          <button onClick={() => onEdit(task)} className="btn-ghost p-1.5 rounded-md" title="Edit task">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
          </button>
          <button onClick={() => onDelete(task.id)} className="p-1.5 rounded-md text-slate-400 hover:text-red-400 hover:bg-red-400/10 transition-all duration-200" title="Delete task">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Description */}
      {task.description && (
        <p className="text-slate-400 text-sm mb-3 leading-relaxed line-clamp-2">{task.description}</p>
      )}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {task.due_date && (
          <span className={`badge ${isOverdue ? 'bg-red-400/10 text-red-400' : 'bg-slate-700/50 text-slate-400'}`}>
            {isOverdue && '⚠ '}
